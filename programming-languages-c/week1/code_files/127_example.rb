...
class MyRational
  ...
  def initialize(num,den=1) # second argument has a default
    ...
    # This is such an unusual decision!
    ...
    # Ruby doesn't allow a self reference to private methods and the parentheses
    # are optional in case of zero or one argument.
  ...
  def to_s2 # using some unimportant syntax and a slightly different algorithm
    dens = ""
    # A better way is to use unless for false conditions
    dens = "/" + @den.to_s unless @den == 1
  ...
  def add! r # mutate self in-place
    # To access same class different instance values, the accessor needs to be
    # made protected. This will make it available for all instances of the same
    # class.
  ...
  # private methods are accessible only to that instance.
  private
