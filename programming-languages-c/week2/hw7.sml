...
fun eval_prog (e,env) =
    case e of
       ...
       | Shift(dx, dy, e1) =>
           (* Evaluating the subexpression in the shift expression should return
            * some gemoetry value. If it doesn't, we will raise an exception. *)
           case eval_prog (e1, env) of
                Point(x1, y2) => Point(x1 + dx, y2 + dy)
              | Line(m, b) => Line(m, b + dy - m * dx)
              | VerticalLine x => VerticalLine(x + dx)
              | LineSegment(x1, y1, x2, y2) => LineSegment(x1 + dx, y1 + dy, x2 + dx, y2 + dy)
              | NoPoints => NoPoints
              | _ => raise BadProgram("Invalid value after evaluating a 'Shift' expression")
