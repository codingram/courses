...
;; Produces true if the winner of the bracket beat the listed teams in reverse order
;; on the way to winning the current round

(define (ordered-wins? bracket list-of-team)
  (cond [(empty? list-of-team) true]
        [(false? bracket) false]
        [else
          (if (string=? (bracket-team-lost bracket) (first list-of-team))
            (ordered-wins? (bracket-br-won bracket) (rest list-of-team))
            (ordered-wins? (bracket-br-won bracket) list-of-team))]))
...
(check-expect (ordered-wins? BE empty) true)
(check-expect (ordered-wins? false '("Nemesis")) false)
(check-expect (ordered-wins? BF '("Ozone")) true)
(check-expect (ordered-wins? BF '("Nemesis")) false)
(check-expect (ordered-wins? BK '("Schwa")) true)
(check-expect (ordered-wins? BL '("Schwa" "Nemesis")) false)
(check-expect (ordered-wins? BN '("Riot" "Phoenix")) true)
(check-expect (ordered-wins? BN '("Riot" "Capitals" "Traffix")) false)
