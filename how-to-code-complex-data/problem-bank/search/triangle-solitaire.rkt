...
;; interp. true represents a cell which contains the peg,
;;         false represents the absence of the peg
...
;;         x represents the position on a specific row (zero-indexed)
;;         y represents the specific row on the board (zero-indexed)
    ...
    [(define possible-jumps (potential-jumps board-size))
     ...
     (define (helper-board board)
       (cond [(solved? board) (list board)]
             [else
               (let ([result (helper-list-of-board (next-boards board possible-jumps))])
                 (if (not (false? result))
                   (cons board result)
                   false))]))
...
(define (construct-board board-size starting-position)
  (if (< board-size 4)
    (error 'construct-board "contract violation\n  expected: >= 4\n  given: " board-size)
    (set-cell-content
      (build-list (sum-of-n board-size) (lambda (p) true))
      starting-position
      false)))
...
(define (potential-jumps board-size)
  (local
    [
     ...
     ; Append the original list of Jump with each of them reversed.
     (define (add-reverse-jumps jump-list)
       (append jump-list
               (map
                 (lambda (jump)
    ...
    (add-reverse-jumps
      (filter valid-potential-jump?
              ...
              (map (lambda (combination)
                     (apply make-jump (map index->position combination)))
...
(define (valid-potential-jump? jump)
  (let ([from-pos-x (position-x (jump-from jump))]
    ...
    ; If all three holes are in the same row
    (if (= from-pos-y over-pos-y to-pos-y)
      ; Check all three holes are one after another
      (= (abs (- from-pos-x over-pos-x)) (abs (- over-pos-x to-pos-x)) 1)
      ; else, check all three holes belongs to different row but should be one after another
  ...
  (local
    [
     ...
     (define (helper row-index)
       (let ([possible-row-indexes (board-indexes-in-row row-index)])
...
(define (set-cell-content board position value)
  (cond
    ...
    [(position? position) (list-set board (position->index position) value)]
    [else (list-set board position value)]))
...
(define (animate-solve board-size starting-position)
  (let ([solutions (solve board-size starting-position)])
    (if (false? solutions)
      (printf
        (string-append
          "No solution found: This starting position is not solvable\n"
          "  board size: ~a\n"
          "  starting position: ~a") board-size starting-position)
      (run-movie
        MOVIE-RATE
