...
(define (render-bst bst)
  (cond [(false? bst) MTTREE]
        [else
          (local [(define left-bst (render-bst (node-l bst)))
                  (define right-bst (render-bst (node-r bst)))]
            (above (render-key-val (node-key bst) (node-val bst))
                   (lines (image-width left-bst) (image-width right-bst))
                   (beside left-bst right-bst)))]))
...
(define (lines lw rw)
  (local
    [(define total-width (+ lw rw))
     (define total-height (/ (+ lw rw) 4))
     (define top-center (/ total-width 2))
     (define bottom-left (/ lw 2))
     (define bottom-right (+ lw (/ rw 2)))]
    (add-line
      (add-line (rectangle total-width total-height "solid" "white")  ;background
                top-center 0
                bottom-left total-height
                "black")
      top-center 0
      bottom-right total-height
      "black")))
