...
def play_hand(hand, word_list):
    ...
    while calculate_handlen(hand) > 0:
        ...
        if playerInput == ".":
            ...
            print(
                f'"{playerInput}" earned {playerInputScore} points. Total: {totalScore} points.'
            )
...
def readVal(valType, requestMsg, errorMsg):
    ...
    """
    while True:
        val = input(f"{requestMsg}: ")
        try:
            return valType(val)
        except ValueError:
            print(f'"{val}": {errorMsg}\n')
...
def inputHandling(requestMsg, inputOption, errorMsg="Invalid input."):
    ...
    while True:
        ...
        if userInput not in optionList:
            print(f'"{userInput}": {errorMsg}\n')
            continue
        elif userInput in optionList:
            return userInput
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    totalSeriesCount = 0
    numHands = NH = readVal(
        int, "Enter total number of hands you want to play", "Invalid input."
    )
    ...
    while numHands > 0:
        ...
        if subChoice == "y":
            subLetter = inputHandling(
                "Which letter would you like to replace", string.ascii_letters
            )
    ...
    print(f"{LINE_SEP}\nTotal score over {NH} hands: {totalSeriesCount} \n{LINE_SEP}")
